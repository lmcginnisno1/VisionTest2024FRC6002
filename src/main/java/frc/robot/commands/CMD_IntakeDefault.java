...
public class CMD_IntakeDefault extends Command {
  ...
  @Override
  public void execute() {
    if(m_robot.m_variables.isRobotState(RobotState.ReadyToIntake) && m_robot.m_intake.getGroundIntakeCurrent() > 13){
            m_robot.m_intake.setIndexerVelocity(1000);
      }
      if(m_robot.m_variables.isRobotState(RobotState.ReadyToIntake) && m_robot.m_intake.getIntakeSensor()){
          ...
          m_robot.m_DriverControllerHI.setRumble(RumbleType.kBothRumble, 1);
          new SequentialCommandGroup(
            new InstantCommand(()-> m_robot.m_variables.setRobotState(RobotState.TransitioningToHome))
            ,new CMD_ArmHome(m_robot.m_arm).withTimeout(1)
            ,new WaitCommand(0.5)
